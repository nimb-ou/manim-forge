...
def jobs() -> list[Job]:
    ...
    return [
        ...
        local("synth-plans", "synth_plans.py", d / "plan_synth.jsonl",
              ...
              done_when=lambda: (d / "synth_done").exists(),
              note="teacher-written arcs for corpus topics"),
        local("synth-kit-0", "synth_kit_beats.py.*--shard 0/3",
              ROOT / "data" / "kit" / "kit_scenes.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "synth_kit_beats.py"),
               "--limit", "5000", "--shard", "0/3"],
              log=ROOT / "data" / "logs" / "synth_kit_0.log",
              note="teacher kit beats, shard 0 of 3",
              stallable=False,
              # Paused while the Mac trains: touch data/kit/pause_0.
              done_when=lambda: (ROOT / "data" / "kit" / "pause_0").exists()),
        local("synth-kit-1", "synth_kit_beats.py.*--shard 1/3",
              ROOT / "data" / "kit" / "kit_scenes.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "synth_kit_beats.py"),
               "--limit", "5000", "--shard", "1/3"],
              log=ROOT / "data" / "logs" / "synth_kit_1.log",
              note="teacher kit beats, shard 1 of 3",
              stallable=False,
              # Paused while the Mac trains: touch data/kit/pause_1.
              done_when=lambda: (ROOT / "data" / "kit" / "pause_1").exists()),
        local("synth-kit-2", "synth_kit_beats.py.*--shard 2/3",
              ROOT / "data" / "kit" / "kit_scenes.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "synth_kit_beats.py"),
               "--limit", "5000", "--shard", "2/3"],
              log=ROOT / "data" / "logs" / "synth_kit_2.log",
              note="teacher kit beats, shard 2 of 3",
              stallable=False,
        ...
        local("fix-arcs", "fix_arcs.py", d / "plan_synth_fixed.jsonl",
              ...
              log=ROOT / "data" / "logs" / "fix_arcs.log",
              note="teacher corrects arcs the referee flagged",
              stallable=False),
        local("judge-fixed", "judge_arcs.py.*fixed",
              d / "arc_judgements_fixed.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "judge_arcs.py"),
               "--src", str(d / "plan_synth_fixed.jsonl"),
               "--out", str(d / "arc_judgements_fixed.jsonl"),
               "--done-marker", "judge_fixed_done", "--follow",
               "--wait-for", "fix_done", "--provider", "gemini"],
              done_when=lambda: (d / "judge_fixed_done").exists(),
              log=ROOT / "data" / "logs" / "judge_fixed.log",
              note="referee re-checks the corrected arcs",
              stallable=False),
        # gemini only. magistral-medium passed ten arcs gemini had flagged,
        # "1/2 * 1/4 = 1/2" among them; a judge that says OK to everything
        ...
        local("synth-topics", "synth_topics.py", d / "topics.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "synth_topics.py"),
               "--target", "1500"],
              done_when=lambda: lines_in(d / "topics.jsonl") >= 1500,
              log=ROOT / "data" / "logs" / "synth_topics.log",
              note="new explainer topics, phrased as users ask"),
        local("synth-requests", "synth_requests.py",
              d / "request_variants.jsonl",
              [str(PY), "-u", str(ROOT / "scripts" / "synth_requests.py"),
               "--follow"],
              log=ROOT / "data" / "logs" / "synth_requests.log",
              note="short and one-sentence versions of each request",
              stallable=False),
        ...
        local("collect-coder2", "collect_adapter.*coder2",
              ...
              stallable=False),
        local("collect-coder3", "collect_adapter.*coder3",
              ROOT / "adapters" / "mlx-coder3" / "adapters.safetensors",
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-coder-sft",
               "--peft", str(ROOT / "adapters" / "kaggle-coder3"),
               "--mlx", str(ROOT / "adapters" / "mlx-coder3")],
              done_when=lambda: (ROOT / "adapters" / "mlx-coder3"
                                 / "adapters.safetensors").exists(),
              log=ROOT / "data" / "logs" / "collect_coder3.log",
              note="waits, downloads, converts, verifies coder v3",
              stallable=False),
        local("collect-kit5", "collect_adapter.*coder5-kit",
              ROOT / "adapters" / "mlx-coder5-kit" / "adapters.safetensors",
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-kit-sft",
               "--peft", str(ROOT / "adapters" / "kaggle-coder5-kit"),
               "--mlx", str(ROOT / "adapters" / "mlx-coder5-kit")],
              done_when=lambda: (ROOT / "adapters" / "mlx-coder5-kit"
                                 / "adapters.safetensors").exists(),
              log=ROOT / "data" / "logs" / "collect_kit5.log",
              note="waits, downloads, converts, verifies the kit coder",
              stallable=False,
              ready=lambda: (ROOT / "data" / "kit" / "kit5_pushed").exists()),
        local("collect-grpo6", "collect_adapter.*coder6-grpo",
              ROOT / "adapters" / "mlx-coder6-grpo" / "adapters.safetensors",
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-coder-grpo",
               "--peft", str(ROOT / "adapters" / "kaggle-coder6-grpo"),
               "--mlx", str(ROOT / "adapters" / "mlx-coder6-grpo")],
              done_when=lambda: (ROOT / "adapters" / "mlx-coder6-grpo"
                                 / "adapters.safetensors").exists(),
              log=ROOT / "data" / "logs" / "collect_grpo6.log",
              note="waits, downloads, converts, verifies the GRPO kit coder",
              stallable=False,
              ready=lambda: (ROOT / "data" / "kit" / "grpo6_pushed").exists()),
        local("collect-coder4", "collect_adapter.*coder4",
              ROOT / "adapters" / "mlx-coder4" / "adapters.safetensors",
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-coder-sft",
               "--peft", str(ROOT / "adapters" / "kaggle-coder4"),
               "--mlx", str(ROOT / "adapters" / "mlx-coder4")],
              done_when=lambda: (ROOT / "adapters" / "mlx-coder4"
                                 / "adapters.safetensors").exists(),
              log=ROOT / "data" / "logs" / "collect_coder4.log",
              note="waits, downloads, converts, verifies coder v4 (gated mix)",
              stallable=False,
              ready=lambda: (ROOT / "data" / "coder_v4_pushed").exists()),
        local("collect-planner4", "collect_adapter.*planner4",
              ROOT / "adapters" / "mlx-planner4" / "adapters.safetensors",
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-planner-sft",
               "--peft", str(ROOT / "adapters" / "kaggle-planner4"),
               "--mlx", str(ROOT / "adapters" / "mlx-planner4")],
              done_when=lambda: (ROOT / "adapters" / "mlx-planner4"
                                 / "adapters.safetensors").exists(),
        ...
        local("collect-planner3", "collect_adapter.*planner3",
              ...
              [str(PY), "-u", str(ROOT / "scripts" / "collect_adapter.py"),
               "--kernel", "nimbou/manim-forge-planner-sft",
               "--peft", str(ROOT / "adapters" / "kaggle-planner3"),
               "--mlx", str(ROOT / "adapters" / "mlx-planner3")],
              done_when=lambda: (ROOT / "adapters" / "mlx-planner3"
                                 / "adapters.safetensors").exists(),
              log=ROOT / "data" / "logs" / "collect_planner3.log",
              note="waits, downloads, converts, verifies planner v3",
              stallable=False),
